...
class HyperConvR(BaseClass):
    def __init__(self, dataset, device, **kwargs):
        ...
        assert self.emb_dim == self.reshape[0] * self.reshape[1]
        ...
        self.bn3 = torch.nn.BatchNorm1d(self.emb_dim)
        self.filtered = [(self.reshape[0] - self.kernel_size[0]) // self.stride + 1, 
                         (self.reshape[1] - self.kernel_size[0]) // self.stride + 1]
        fc_length = self.conv_out_channels * self.filtered[0] * self.filtered[1]
        self.fc1 = torch.nn.Linear(self.emb_dim, self.max_arity * self.kernel_size[0] * self.kernel_size[1] * self.conv_out_channels)
        self.fc2 = torch.nn.Linear(fc_length, self.emb_dim)
    ...
    def convolve(self, e_idx, r):
        ...
        e = self.bn0(e).view(1, -1, *self.reshape)
        e = self.input_drop(e)

        r = r.reshape(-1, 1, *self.kernel_size)
    ...
    def forward(self, r_idx, e1_idx, e2_idx, e3_idx, e4_idx, e5_idx, e6_idx, ms, bs):
        ...
        params = self.fc1(r).view(batch_size, self.max_arity, -1)

        e1 = self.convolve(e1_idx, params[:, 0, :]) * ms[:,0].view(-1, 1) + bs[:,0].view(-1, 1)
        e2 = self.convolve(e2_idx, params[:, 1, :]) * ms[:,1].view(-1, 1) + bs[:,1].view(-1, 1)
        e3 = self.convolve(e3_idx, params[:, 2, :]) * ms[:,2].view(-1, 1) + bs[:,2].view(-1, 1)
        e4 = self.convolve(e4_idx, params[:, 3, :]) * ms[:,3].view(-1, 1) + bs[:,3].view(-1, 1)
        e5 = self.convolve(e5_idx, params[:, 4, :]) * ms[:,4].view(-1, 1) + bs[:,4].view(-1, 1)
        e6 = self.convolve(e6_idx, params[:, 5, :]) * ms[:,5].view(-1, 1) + bs[:,5].view(-1, 1)
...
class HyperConvE(BaseClass):
    def __init__(self, dataset, device, **kwargs):
        ...
        for _ in range(self.max_arity):
            conv_layer = torch.nn.Conv2d(1, self.conv_out_channels, self.kernel_size, self.stride, 0, bias=kwargs['conv']['use_bias']).to(self.device)
    ...
    def convolve(self, e_idx, r, pos):
        ...
        conv = self.conv_list[pos]
        x = conv(x)  # (batch_size, conv_out_channels, x2, y2)
        x = self.bn1(x)
        x = F.relu(x)
        x = self.feature_map_drop(x)
